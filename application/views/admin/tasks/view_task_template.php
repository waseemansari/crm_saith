        ...
        <hr />
            <div class="tw-flex tw-justify-between tw-items-center" id="material">
                ...
                <div class="tw-flex tw-items-center tw-space-x-2">
                    <button type="button" 
                        data-toggle="tooltip"
                        data-title="<?php echo _l('add_edit_inventory'); ?> "
                        class="tw-inline-flex tw-bg-transparent tw-border-0 tw-p-1.5 hover:tw-bg-neutral-100 tw-text-neutral-500 hover:tw-text-neutral-700 focus:tw-text-neutral-700 tw-rounded-md ltr:tw-ml-2 rtl:tw-mr-2"
                        onclick="toggleInventoryList(<?= e($task->id); ?>)"
                        >
                        <i class="fa-solid fa-plus"></i>
                    </button>
                </div>
              ...
              <div class="form-group inventory_list_div hidden">
                <?php  $items =  $this->db->query('select id,description,unit,rate from tblitems')->result_array(); ?>
                   ...
                   <form id="itemsForm">
                        ...
                        <button type="submit" id="saveBtn" class="btn btn-primary" style="display:none; margin-top:10px">
                                Save All Items
                            </button>               
                    </form>
            </div>
                ...
                <?php if (!empty($task_inventories)) { 
                    ...
                    <?php foreach ($task_inventories as $inv) { ?>
                        <div class="tw-flex tw-items-center tw-justify-between tw-border-b tw-py-2" id="inv_item_<?= $inv['id']; ?>" data-cost="<?= $inv['qty'] * $inv['rate']; ?>">
                            ...
                            <div class="tw-flex tw-items-center">
                                <!-- Edit button -->
                                <a href="#" onclick="editInventoryItem(<?= $inv['id']; ?>); return false;" class="tw-text-blue-500 tw-ml-2">
                                    <i class="fa-regular fa-pen-to-square"></i>
                                </a>

                                <!-- Delete button -->
                                <a href="#" onclick="deleteInventoryItem(<?= $inv['id']; ?>); return false;" class="tw-text-neutral-500 tw-ml-2">
...
function deleteInventoryItem(id) {
    ...
    fetch('<?= admin_url('tasks/delete_inventory_item'); ?>/' + id, {
        method: 'DELETE',
...
function saveInventoryItem(id) {
    ...
    $.ajax({
        url: admin_url + "tasks/edit_inventory_item/" + id,
        type: "POST",
        data: formData,
...
<script>
  function toggleInventoryList() {
    ...
    div.classList.toggle("hidden"); // Tailwind "hidden" class hides the div
  }

  function toggleSaveButton() {
    if ($("#selected_items .item-box").length > 0) {
        $("#saveBtn").show();
    } else {
        $("#saveBtn").hide();
    }
...
<script>
    ...
    $("#itemsForm").on("submit", function(e) {
        ...
        $.ajax({
            url: admin_url + "tasks/add_inventory_item",
            type: "POST",
            data: formData,
            processData: false,   // required for files
            contentType: false,   // required for files
