...
class Proposals_model extends App_Model
{
     ...
     */
    public function add_comment($data, $file='',$client = false)
    {
        ...
        if ($insert_id) {
            ...
            $uploadDir = FCPATH . 'uploads/proposal_comments/'; // Save inside "uploads" folder
            ...
            $fileTmp   = is_array($_FILES['file']['tmp_name']) ? $_FILES['file']['tmp_name'] : [$_FILES['file']['tmp_name']];

            foreach ($fileNames as $index => $name) {
                $tmpName = $fileTmp[$index];
